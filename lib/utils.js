...
function cycleTips(prefix, tips, callback) {
	process.stdout.clearLine();
	prefix += ': ';
	process.stdout.write(prefix.cyan);
	process.stdout.write(tips.cyan);
	setTimeout(function() {
		for (var i = 0; i < prefix.length + tips.length; i++) {
			process.stdout.write('\b');
		}
		// process.stdout.moveCursor(prefix.length + tips.length, 0);
		callback && callback();
	}, 1000);
}
