...
function insertWithFilter(totalPage, count, callback) {
	var sum = 0;
	function fetch(index, totalPage, addCount, callback) {
			...
			request(templateUrl.replace('{0}', index), function(err, res, body) {
				...
				async.forEachSeries(contentArray, function(o, callback) {
					dao.count(o, function(err, rows, fields) {
						...
						if (rows[0].COUNT == 0) {
							dao.insert(o, function(err, rows, fields) {
								if (err) {
									callback && callback(err);
									return false;
								}
					...
					utils.cycleTips({
						...
						eraseLength: function() {
							if (sum > 0) {
								return (parseInt((sum - addCount) * 100 / count) + '%').length;
							} else {
								return (parseInt(100 / count) + '%').length;
							}
						},
						callback: function() {
							sum += tempCount;
							fetch(++index, totalPage, tempCount, callback);
						}
					});
				});
			});
